	...
	if (m_iTicks++ < CParams::iNumTicks)
	{
		for (int i=0; i<m_NumSweepers; ++i)
		{
			...
			if (!m_vecSweepers[i].Update(m_ObjectsVB))
			{
				...
				MessageBoxA(m_hwndMain, "Wrong amount of NN inputs!", "Error", MB_OK);
...
void CController::Render(HDC &surface)
{
	...
	TextOut(surface, 5, 0, LPCWSTR(s.c_str()), s.size());
	...
	SelectObject(surface, m_BluePen);//原画笔 m_OldPen
...
{
    string s = "Generation:              " + itos(m_iGenerations);
	  TextOut(surface, 5, 25, LPCWSTR(s.c_str()), s.size());
    
    s = "Num Species:          " + itos(m_pPop->NumSpecies());
	TextOut(surface, 5, 45, LPCWSTR(s.c_str()), s.size());

    s = "Best Fitness so far: " + ftos(m_pPop->BestEverFitness());
	TextOut(surface, 5, 5, LPCWSTR(s.c_str()), s.size());
}
