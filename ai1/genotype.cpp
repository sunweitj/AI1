...
{  
  ...
  MessageBoxA(NULL, "Error in CGenome::GetElementPos", s.c_str(), MB_OK);
                        ...
                        int          NumTrysToFindOldLink)
{
  ...
  else
  {
    ...
    {
      MessageBoxA(NULL, "Error in CGenome::AddNeuron", "Problem!", MB_OK);
...
bool CGenome::CreateFromFile(const char* szFileName)
{
  ...
  if (!in)
  {
    MessageBoxA(NULL, "Cannot find genome file!", "error", MB_OK);
