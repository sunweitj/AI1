================================================================================
    MICROSOFT 基础类库 : ai1 项目概述
===============================================================================

应用程序向导已为您创建了此 ai1 应用程序。此应用程序不仅演示 Microsoft 基础类的基本使用方法，还可作为您编写应用程序的起点。

本文件概要介绍组成 ai1 应用程序的每个文件的内容。
...
ai1.h
    这是应用程序的主头文件。
    其中包括其他项目特定的标头（包括 Resource.h），并声明 Cai1App 应用程序类。

ai1.cpp
    这是包含应用程序类 Cai1App 的主应用程序源文件。

ai1.rc
    这是程序使用的所有 Microsoft Windows 资源的列表。它包括 RES 子目录中存储的图标、位图和光标。此文件可以直接在 Microsoft Visual C++ 中进行编辑。项目资源包含在 2052 中。

res\ai1.ico
    这是用作应用程序图标的图标文件。此图标包括在主资源文件 ai1.rc 中。
...
res\Toolbar.bmp
    此位图文件用于为工具栏创建平铺图像。
    初始工具栏和状态栏在 CMainFrame 类中构造。使用资源编辑器编辑此工具栏位图，并更新 ai1.rc 中的 IDR_MAINFRAME TOOLBAR 数组以添加工具栏按钮。
/////////////////////////////////////////////////////////////////////////////
...
ai1Doc.h、ai1Doc.cpp - 文档
    这些文件包含 Cai1Doc 类。编辑这些文件以添加特殊文档数据并实现文件保存和加载（通过 Cai1Doc::Serialize）。

ai1View.h、ai1View.cpp - 文档视图
    这些文件包含 Cai1View 类。
    Cai1View 对象用于查看 Cai1Doc 对象。

res\ai1Doc.ico
    这是一个图标文件，用作 Cai1Doc 类的 MDI 子窗口的图标。此图标包括在主资源文件 ai1.rc 中。
...
StdAfx.h, StdAfx.cpp
    这些文件用于生成名为 ai1.pch 的预编译头 (PCH) 文件和名为 StdAfx.obj 的预编译类型文件。
...
0
	/*
		...
		//ar.Flush();
		DWORD count = 0;// ar.ReadCount();
		char ass[2] = { 0 };	//计数比对
		while (ar.Read(lpszLine, 1))   //ar.Read(lpszLine,1);
		{
			


			tmp.Format(_T("%s"), lpszLine);
			//	AfxMessageBox(tmp);
			strLine = strLine + tmp;
...
/////////////////////////////////////////////////////////////////////
			...
			if (tmp == "\n")
			{
				...
				//char *t = strLine.GetBuffer(strLine.GetLength());
				for (int i = 0; i < strLine.GetLength(); i++)
				{
					lpszLine[i] = strLine[i];
				}
				//strcpy_s(lpszLine, (LPCSTR)(LPCTSTR)strLine);
				
				char* p = strtok_s(lpszLine, "\t\r\n", &buf);
				while (p != NULL)
				{
					Gollist.tim = p;		p = strtok_s(NULL, "\t\r\n", &buf);
					if (p = NULL) break;
					Gollist.dat = p;		p = strtok_s(NULL, "\t\r\n", &buf);
					
					Gollist.open = atof(p); p = strtok_s(NULL, "\t\r\n", &buf);
