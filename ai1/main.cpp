	...
	{	
      ...
      PAINTSTRUCT ps;
		  ...
		  BeginPaint(hwnd, &ps);
			...
			//-----------------------------------实验坐标映射函数-----------------------------
			//SetMapMode(ps.hdc, MM_ANISOTROPIC); SetWindowOrgEx(ps.hdc, 0, 0)
			//-----------------------------------------------------------------------
			RECT *rc;
			rc = new(RECT );
			LPPOINT lpPoint;
			...
			SetWindowOrgEx(ps.hdc, 0, 0, nullptr);
			SetWindowExtEx(ps.hdc, (rc->right - rc->left), (rc->bottom - rc->top), NULL);
			SetViewportOrgEx(ps.hdc, 0, 0, nullptr);
			SetViewportExtEx(ps.hdc, (rc->right - rc->left), (rc->bottom - rc->top), nullptr);
			HPEN pen = CreatePen(PS_DASH, 1, RGB(0, 0, 255));
			HGDIOBJ oldpen = SelectObject(ps.hdc, pen);
			MoveToEx(ps.hdc, 20, 20, nullptr);
			LineTo(ps.hdc, 200, 200);
